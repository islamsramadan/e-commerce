...
module.exports.addProduct = (req, res, next) => {
	...
	newProduct
		.save()
		.then((data) => {
			res.status(201).json({ status: 'success', message: 'product added', data: data });
		})
		.catch((err) => next(err));
};
...
module.exports.updateProduct = (req, res, next) => {
	...
	if (req.role === 'admin' || (req.role === 'business' && req.id == req.body.id)) {
...
module.exports.deleteOneProduct = (req, res, next) => {
	if (req.role === 'admin' || (req.role === 'business' && req.id == req.params.id)) {
...
exports.filterProducts = async (req, res, next) => {
	try {
		...
		if (!filterKey) throw new Error('Filter criteria must be Category, Price or Rating');
		...
		switch (filterKey) {
			case 'category':
				...
				if (!matchingCategory) throw new Error('Couldnt find category with that name!');
			...
			case 'rating':
				if (isNaN(req.query.rating)) throw new Error('Rating must be a number!');
				matchingProducts = await Products.find(
					{ rating: req.query.rating, name: { $regex: new RegExp(searchText, 'i') } },
					{ name: 1, price: 1, rating: 1 }
				);
		}
...
exports.getRelatedProducts = async (req, res, next) => {
	try {
		...
		if (!relatedProducts.length) throw new Error('Couldnt find products in that category!');
...
async function getImages(data) {
	...
	data.forEach(async (element) => {
		const filePath = path.join(__dirname, '..', 'images', 'products', element._id.toString());
		let files = await readdir(filePath);
		let bitmap = fs.readFileSync(
			path.join(__dirname, '..', 'images', 'products', element._id.toString(), files[0])
		);
		const image = 'data:image/jpeg;base64,' + Buffer.from(bitmap).toString('base64');
