  ...
  Business.findOne({ userId: req.params.id })
    .then(async (data) => {
      ...
      return await data.save().then((data) => {
        res.status(200).json({ success: true, message: 'img updated' });
      });
    })
    .catch((err) => {
      next(err);
...
module.exports.uploadComRegImg = (req, res, next) => {
  res.status(200).json({ sucess: true, message: 'img updated' });
};

module.exports.getBusinessStatistics = async (req, res, next) => {
  try {
    ...
    const numOfSoldProducts = await orders.count({
      'orderItems.businessId': req.params.id,
    });
